...
describe('auth', () => {
  ...
  it('null session: `auth` mock returns `null` → `getSession()` rejects with message matching `\'Unauthenticated\'`', async () => {
    vi.mocked(auth).mockResolvedValue(null as any)

    await expect(getSession()).rejects.toThrow('Unauthenticated')
  })

  it('missing id: `auth` mock returns `{ user: {} }` → `getSession()` rejects with message matching `\'Unauthenticated\'`', async () => {
